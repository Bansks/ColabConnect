...
public class ConnectionController {
    ...
    public ResponseEntity<?> getConnections(@CookieValue(value = "cc_token", required = false) String token) {
        ...
        // Include all co-members from all projects the user is a member/owner of
        Set<String> connectionUserIds = result.stream()
                .map(m -> (String) m.get("id"))
                .collect(Collectors.toSet());

        List<Project> myProjects = projectRepository.getProjectsByUserId(userId);
        for (Project p : myProjects) {
            List<String> allMemberIds = new ArrayList<>(p.getMemberIds());
            if (!allMemberIds.contains(p.getOwnerId())) {
                allMemberIds.add(p.getOwnerId());
            }
            for (String mid : allMemberIds) {
                if (!mid.equals(userId) && !connectionUserIds.contains(mid)) {
                    User u = userRepository.getUserById(mid);
                    if (u != null) {
                        Map<String, Object> m = new LinkedHashMap<>();
                        m.put("id", u.getId());
                        m.put("name", u.getName());
