...
public class DirectMessageController {
    ...
    public DirectMessageController(ChatRepository chatRepository, ConnectionRepository connectionRepository, ProjectRepository projectRepository, SessionRepository sessionRepository, UserRepository userRepository, SimpMessagingTemplate messagingTemplate) {
    ...
    private boolean areConnected(String uid1, String uid2) {
        if (connectionRepository.areConnected(uid1, uid2)) {
            return true;
        }
        // Check if they share any project
        return projectRepository.getProjectsByUserId(uid1).stream()
                .anyMatch(p -> p.getOwnerId().equals(uid2) || p.getMemberIds().contains(uid2));
    }
    ...
    public ResponseEntity<?> getConversations(
        ...
        // Add all co-members of shared projects
        List<Project> sharedProjects = projectRepository.getProjectsByUserId(userId);
        Set<String> coMemberIds = sharedProjects.stream()
                .flatMap(p -> {
                    List<String> ids = new ArrayList<>(p.getMemberIds());
                    if (!ids.contains(p.getOwnerId())) {
                        ids.add(p.getOwnerId());
                    }
                    return ids.stream();
                })
        ...
        allPartners.addAll(coMemberIds);
        ...
        List<Map<String, Object>> result = allPartners.stream()
                ...
                .map(partnerId -> {
                    ...
                    m.put("connected", areConnected(userId, partnerId));
    ...
    public ResponseEntity<?> getConversation(
        ...
        boolean connected = areConnected(userId, partnerId);
